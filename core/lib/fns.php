...
function pageTitle(){
    if(!empty($_SERVER['QUERY_STRING'])){
        ...
        $base_uri = basename($_SERVER['REQUEST_URI']);
        $base_uri = str_replace("_", " ", $base_uri);
        $base_uri = str_replace("-", " ", $base_uri);
        // check if script is running on localhost, hence load the desired page title
        $iparray = array("127.0.0.1", "::1");

        if(in_array($_SERVER['REMOTE_ADDR'], $iparray)){
            if($base_uri == 'nedmedicare'){
                print 'Welcome to pustera goods locally';
            }
            else{
                print 'pustera goods - '.ucfirst($base_uri);
            }
        }
        else{
            if(empty($base_uri)){
                print 'Welcome to pustera';
            }
            else{
                print 'pustera goods - '.ucfirst($base_uri);
            }
...
function currencyRate($from,$to){
    ...
    $get = file_get_contents("https://www.google.com/finance/converter?a=$encode_amount&from=$from&to=$to");
...
function currencyConvert($rate, $amount){
    $converted_amount = ceil($rate*$amount);
    ...
    if(!empty($_SERVER['QUERY_STRING'])){
        ...
        if($total_pages > 1){
            ...
            for($i = $prev_btns; $i < $page_num; $i++){
                ...
                print str_replace('li', '<li class="'.$addclass.'">', $list);
                ?>
                    <a href="<?php print $fullUrl.$i; ?>">
                    <?php print $i; ?>
                    </a>
            <?php
...
array values:
    ...
    error_msg
*/
function fh_mail($content = array(),$smtp = NULL){
    // include php mailer class
    load('PHPMailer/src/PHPMailer.php');
    load('PHPMailer/src/SMTP.php');
...
// check mail    
if(!$mail->send()){
    throw new Exception($content['error_msg']);
}
}
...
*/
function load_model($class,$type){
    if($type == 'db'){
        $path = "engine/model/$class.php";
    }
    else if($type == 'model'){
        $path = "model/$class.php";
    }
    else if($type == 'helper'){
        $path = "engine/helpers/$class.php";
    }
    else{
        $path = NULL;
    }
    ...
    if($path != NULL){
        ...
        throw new Exception("Unable to load class: Invalid class");
    }
}
...
function brief($text, $return){
    if(count(explode(" ", $text)) > 1){
        $text = array_slice(explode(" ", $text), 0, $return);
        $trimmed_text = NULL;
        for($i=0; $i<=count($text)-1; $i++){
            $text_app = ($i < count($text)-1) ? " " : "...";
            $trimmed_text .= $text[$i].$text_app;
        }
        return $trimmed_text;
    }
    else{
